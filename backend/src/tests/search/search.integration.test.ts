...
vi.mock("../../services/search.services.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../services/search.services.js")>()
  return {
    ...actual,
...
const { manejadorErrores } = await import("../../middlewares/error.middlewares.js")
...
beforeEach(() => {
  ...
  getOSetMock.mockImplementation(async (_key: string, fn: () => Promise<unknown>) => fn())
  checkIPSpikeMock.mockResolvedValue(false)
  enriquecerConDatosLocalesMock.mockImplementation(async (ids: number[]) => {
    const map = new Map<number, { vault_count: number; review_count: number; watchlist_count: number }>()
    if (ids.includes(101)) {
  ...
  consultarTMDBMock.mockImplementation(async (endpoint: string, params: Record<string, string>) => {
    ...
    if (endpoint === "search/movie") {
      ...
      if (query.includes("aftersun")) {
        return {
          results: [createMovieResult(202, "Other Match", 80), createMovieResult(101, "Aftersun", 10)],
          total_pages: 1,
          total_results: 2,
          page: 1,
        }
    ...
    if (endpoint === "search/tv") {
      return {
        results: [{ id: 3001, name: "Aftersun BTS", original_name: "Aftersun BTS", popularity: 12 }],
        total_pages: 1,
        total_results: 1,
        page: 1,
      }
...
describe("search integration", () => {
  it("evita buscar personas para query de titulo comun", async () => {
    const res = await request(app).get("/api/search").query({ q: "fight club", page: 1 })

    expect(res.status).toBe(200)
    expect(Array.isArray(res.body.results)).toBe(true)

    const personCalls = consultarTMDBMock.mock.calls.filter(
      ([endpoint, params]) => endpoint === "search/person" && String(params?.query || "").toLowerCase() === "fight club"
    )
  ...
  it("exhibe debug con desglose de score y metadata", async () => {
    const res = await request(app).get("/api/search/debug").query({ q: "aftersun", page: 1 })
    ...
    expect(res.body.results[0]._score_debug).toHaveProperty("token_source_boost")
  })
})
