...
export const agregarAWatchlistService = async (
  ...
  const movieId = await ensureMovieRefId(
    data.movie_id,
    (data as any).media_type || "movie"
  )
  const yaExiste = await watchlistRepository.exists(userId, movieId)

  if (yaExiste) {
    throw new ConflictError(
      `La obra ${data.movie_id} ya se encuentra en su lista de seguimiento`
    )
  }
...
export const eliminarDeWatchlistService = (
  ...
  findMovieRefIdByCandidate(data.movie_id, (data as any).media_type).then(
    (resolvedMovieId) => {
