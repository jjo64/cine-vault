...
export async function enriquecerConDatosLocales(tmdbIds: number[]): Promise<Map<number, LocalCounts>> {
  if (tmdbIds.length === 0) return new Map()

  // Seleccionamos campos puntuales para no depender de columnas nuevas
  // (ej. slug) cuando la DB local aun no aplico migraciones.
  let refs: Array<{
    tmdb_id: number
    _count: {
      vault: number
...
export type { QueryAnalizado, SearchIntent, LocalCounts }