...
import { ConflictError, ValidationError, NotFoundError } from "../errors/AppErrors.js"
...
export const actualizarPerfilService = async (
  ...
  datos: { username?: string; avatar_url?: string; bio?: string }
    ...
    if (typeof datos.username !== "string" || datos.username.trim().length < 3) {
  ...
  try {
    await prisma.users.update({ where: { id: idUsuario }, data: datosActualizar })
  } catch (error) {
 ...
 */
export const seguirUsuarioService = async (idUsuario: number, idUsuarioSeguir: number) => {
 ...
 * Usa req.user!.user_id del middleware de autenticación (fix: antes usaba Number(req.user!)).
 */
export const dejarDeSeguirUsuarioService = async (idUsuario: number, idUsuarioDejar: number) => {
...
export const obtenerUsuarioPorIdService = async (id: number) => {
  const usuario = await prisma.users.findUnique({
    ...
    select: {
      ...
      _count: { select: { reviews: true, diary_entries: true, watchlist: true,
        follows_follows_follower_idTousers: true, follows_follows_following_idTousers: true } }
    }
  })
  ...
  const usuario = await prisma.users.findUnique({
    ...
    include: {
      follows_follows_following_idTousers: {
        include: { users_follows_follower_idTousers: { select: { id: true, username: true, avatar_url: true } } }
      }
    }
  })
  if (!usuario) throw new NotFoundError("Usuario no encontrado")
  return usuario.follows_follows_following_idTousers.map(f => f.users_follows_follower_idTousers).filter(Boolean)
}
  ...
  const usuario = await prisma.users.findUnique({
    ...
    include: {
      follows_follows_follower_idTousers: {
        include: { users_follows_following_idTousers: { select: { id: true, username: true, avatar_url: true } } }
      }
    }
  })
  if (!usuario) throw new NotFoundError("Usuario no encontrado")
  return usuario.follows_follows_follower_idTousers.map(f => f.users_follows_following_idTousers).filter(Boolean)
}