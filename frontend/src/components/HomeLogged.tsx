...
export default function HomeLogged({ username }: HomeLoggedProps) {
  ...
  const [activeZone, setActiveZone] = useState<ZoneId>(() =>
    resolveZone(searchParams.get("zona")),
  );
  ...
  const [tonightMovie, setTonightMovie] = useState<TonightResponse | null>(
    null,
  );
    ...
    const load = async () => {
      ...
      try {
        ...
        const [
          ...
          fetch(`${API_URL}/api/mentiras/ranking`).then((res) =>
            res.ok ? res.json() : ({} as MentirasRanking),
          ),
        ...
        const diaryData =
          diaryRes.status === "fulfilled" ? diaryRes.value.diary || [] : [];
        const watchlistData =
          watchlistRes.status === "fulfilled" ? watchlistRes.value || [] : [];
        const reviewsData =
        ...
        if (forYouRes.status === "fulfilled") {
          const movies = (forYouRes.value.items || []).filter(
            (i: any) => i.type === "media",
          ) as ForYouMovieItem[];
          ...
          followingUsers.map(async (user) => {
            try {
              ...
              return {
                ...
                time: relativeLabel(
                  first.watched_date || first.review?.created_at || null,
                ),
        ...
        setFollowingActivity(
          activityRaw.filter((item): item is FollowingActivityItem =>
            Boolean(item),
          ),
        );
        ...
        for (const entry of diaryData)
          if (entry.tmdb_id) tmdbIds.add(entry.tmdb_id);
        ...
        for (const item of reviewsByFollowing.flat())
          if (item.tmdbId) tmdbIds.add(item.tmdbId);
          ...
          ids.map(async (tmdbId) => {
            try {
              ...
              const year = data.release_date
                ? Number(String(data.release_date).slice(0, 4))
                : null;
              const overview =
        ...
        setMetaByTmdb(
          Object.fromEntries(
            metaEntries.filter((entry): entry is readonly [number, MovieMeta] =>
              Boolean(entry),
            ),
          ),
        );
  ...
  const tonightFilm = useMemo(() => {
    ...
    return {
      ...
      duration: "Calculada",
      synopsis:
        media.reason || "Recomendación especial calculada para ti esta noche.",
      points: 40,
      genres:
        media.weather_context === "rainy"
          ? ["Lluvia", "Misterio"]
          : ["Esta noche", "Descubrimiento"],
      posterUrl: media.poster_path
        ? `https://image.tmdb.org/t/p/w500${media.poster_path}`
        : "/no-poster.svg",
      backdropUrl: media.poster_path
  ...
  const vaultCards = useMemo(() => {
    return vault.slice(0, 3).map((item, index) => {
      ...
      const meta = tmdbId ? metaByTmdb[tmdbId] : null;
      const title =
        item.movie_info?.title || meta?.title || `Pelicula ${item.movie_id}`;
      const type =
        index % 3 === 0 ? "Reflexion" : index % 3 === 1 ? "Edit" : "Critica";
      return {
        id: item.movie_id,
        type,
        title,
  ...
  const weekStats = useMemo(() => {
    const normalizedProfile = encodeURIComponent(
      (username || greetingName).trim().toLowerCase(),
    );
    ...
    return [
      {
        num: String(diary.length),
        label: "Peliculas",
        icon: <Film size={16} />,
      },
      {
        num: String(reviews.length),
        label: "Reseñas",
      ...
      },
      {
        num: String(
          Math.min(
            7,
            Math.max(1, Math.floor((diary.length + watchlist.length) / 2)),
          ),
        ),
        label: "Dias de racha",
        icon: <Flame size={16} />,
      ...
      {
        num: String(reviews.length * 40 + diary.length * 15),
        label: "Puntos",
        icon: <Trophy size={16} />,
      },
    ];
  ...
  const directorCards = useMemo(() => {
    return directors.slice(0, 6).map((director) => ({
      id: director.id,
      name: director.name,
  ...
  const renderEntrada = () => (
    ...
    >
      ...
      >
        <SectionLabel link="Ver todo" linkHref="/for-you">
          ...
          <em style={{ fontStyle: "italic", color: C.accent, marginLeft: 4 }}>
            {diary[0]?.movie_info?.title || "tu ultima pelicula"}
          </em>
        </SectionLabel>
        <div
          style={{
        ...
        >
          {becauseYouWatched.map((film) => (
            <Link
              key={film.movieId}
              to={movieHref(film.movieId, film.tmdbId, film.title)}
      ...
      <motion.div
        ...
        style={{ marginBottom: 56 }}
      >
        <SectionLabel link="Ver actividad" linkHref="/feed">
          Tus seguidos vieron
        </SectionLabel>
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          {followingActivity.map((item, i) => (
            ...
            >
              ...
              <Link
                to={movieHref(item.movieId, item.tmdbId, item.film)}
                className="hl-activity-poster-link"
                style={{ textDecoration: "none" }}
              >
                <div
                  style={{
              ...
              <div>
                ...
                <span
                  style={{ fontFamily: SANS, fontSize: 12, color: C.textSoft }}
                >
                  {" "}
                  vio{" "}
                </span>
                ...
                <div style={{ display: "flex", gap: 2, marginTop: 4 }}>
                  {[1, 2, 3, 4, 5].map((s) => (
                    <span
                      ...
                      style={{
                        fontSize: 10,
                        color: s <= item.rating ? C.gold : C.textMuted,
                      }}
                    >
                      ★
                    </span>
          ...
          {followingActivity.length === 0 ? (
            ...
            >
              Todavia no hay actividad visible de usuarios seguidos.
            </div>
          ) : null}
        </div>
      </motion.div>
            ...
            >
              ...
              <div>
                <div className="hl-feed-header">
                  ...
                  >
                    <span
                      className="hl-feed-action-label"
                      style={{
                        fontFamily: SANS,
                        fontSize: 11,
                        color: C.textMuted,
              ...
              >
                <button
                  ...
                  aria-label={
                    likedFeedIds.has(post.id)
                      ? `Quitar like (${post.likes} likes)`
                      : `Dar like (${post.likes} likes)`
                  }
      ...
      >
        ...
        <div className="hl-vault-grid">
          {vaultCards.map((item) => (
            <Link
              key={item.id}
              to={movieHref(item.id, item.tmdbId, item.title)}
              style={{ textDecoration: "none" }}
            >
              <motion.div
                whileHover={{ y: -3 }}
            ...
            {diaryHighlights.map((entry) => (
              ...
              >
                ...
                <div>
                  ...
                  >
                    <span
                      style={{ fontFamily: SERIF, fontSize: 17, color: C.text }}
                    >
                      {entry.film}
                    </span>
                    <div style={{ display: "flex", gap: 2 }}>
                      {[1, 2, 3, 4, 5].map((s) => (
                        <span
                          ...
                          style={{
                            fontSize: 10,
                            color: s <= entry.rating ? C.gold : C.textMuted,
                          }}
                        >
                          ★
                        </span>
            ...
            {[
              {
                icon: "🎞️",
                title: "Maratonista",
                desc: "5 peliculas en una semana",
              },
              {
                icon: "✍️",
                title: "Critica en desarrollo",
                desc: "50 reseñas escritas",
              },
              {
                icon: "🕯️",
                title: "Ritual nocturno",
                desc: "7 noches seguidas",
              },
            ].map((b) => (
              <div
                key={b.title}
              ...
              >
                ...
                <div>
                  <div
                    style={{ fontFamily: SANS, fontSize: 12, color: C.text }}
                  >
                    {b.title}
                  </div>
                  <div
  ...
  const renderVitrina = () => (
    ...
    >
      ...
      <motion.div
        ...
        style={{ marginTop: 40 }}
      >
        <SectionLabel link="Abrir ranking" linkHref="/mentiras">
          Mentiras de la comunidad
        </SectionLabel>
        <Link
          to="/mentiras"
          style={{ textDecoration: "none", display: "block" }}
        >
          ...
          >
            ...
            >
              Ranking vivo de confesiones y finalizacion real dentro de la
              comunidad.
            </div>
          </motion.div>
        </Link>
  ...
  return (
    ...
    >
          ...
          <div className="hl-greeting-stats">
            <div className="hl-stat-item">
              ...
              <span style={{ color: C.text }}>
                {Math.min(9, Math.max(1, diary.length))}
              </span>{" "}
              <span className="hl-stat-label">dias de racha</span>
            </div>
            <div className="hl-stat-item">
              <Trophy size={12} color={C.gold} />{" "}
              <span style={{ color: C.text }}>
                {reviews.length * 40 + diary.length * 15}
              </span>{" "}
              <span className="hl-stat-label">puntos</span>
            </div>
            <div className="hl-stat-item">
              ...
              <span style={{ color: C.text }}>
                {Math.min(
                  ...
                  Math.max(
                    1,
                    Math.floor((reviews.length + watchlist.length) / 3),
                  ),
                )}
              </span>{" "}
              <span className="hl-stat-label">insignias</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {ZONES.map((zone, i) => {
          ...
          return (
            <motion.button
              ...
              className={`hl-zone-btn ${isActive ? "active" : ""} ${i < 2 ? "with-border" : ""}`}
                ...
                <div className="hl-zone-dots-mobile">
                  {ZONES.map((z) => (
                    <div
                      ...
                      className={`hl-zone-dot-btn ${activeZone === z.id ? "active" : ""}`}
                    />
                  ))}
                </div>
              </div>
      ...
      <div className="hl-main-content">
        ...
        {!loading && !hasData ? (
          <div style={{ color: C.textSoft, marginBottom: 16 }}>
            No hay actividad suficiente todavia.
          </div>
        ) : null}
      ...
      >
        ...
        >
          &quot;Toda gran coleccion empieza con una.&quot;
        </div>
      </footer>
    </div>
  );
