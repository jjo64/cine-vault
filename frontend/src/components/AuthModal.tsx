...
const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handlePrimarySubmit = async (e: React.FormEvent) => {
    ...
    const endpoint =
      step === "login" ? "/api/auth/login" : "/api/auth/register";
  ...
  const handleTwoFactorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setHasSubmitError(false);
      const accessToken = await verifyTwoFactorLogin(
        twoFactorCode.trim(),
        tempToken,
        rememberDevice,
      );
      ...
      setHasSubmitError(true);
      const message =
        err instanceof Error ? err.message : "No se pudo verificar 2FA";
      notify.loginError(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {step === "login"
            ? "Formulario para iniciar sesión en CineVault"
            : step === "register"
              ? "Formulario para crear una cuenta en CineVault"
          ...
          <>
            ...
            <div className="auth-toggle-text">
              {step === "login" ? (
                <>
                  ¿No tienes cuenta?{" "}
                  <span onClick={() => setStep("register")}>Regístrate</span>
                </>
              ) : (
                <>
                  ¿Ya tienes cuenta?{" "}
                  <span onClick={() => setStep("login")}>Inicia Sesión</span>
                </>
              )}
            </div>
          </>
