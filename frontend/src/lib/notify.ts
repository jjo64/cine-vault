...
export const notify = {
  ...
  emailVerified: () =>
    toast.success("Email verificado. ¡Bienvenido a CineVault!"),
  ...
  watchlistAdd: (title: string) =>
    toast.success(`"${title}" agregada a tu watchlist.`),
  watchlistRemove: (title: string) => toast(`"${title}" quitada de watchlist.`),
  favoritesAdd: (title: string) =>
    toast.success(`"${title}" agregada a favoritos.`),
  ...
  fromSocket: (notif: {
    ...
    switch (notif.type) {
      case "follow":
        toast(`${actor} empezó a seguirte.`);
        break;
      case "like":
        toast(`${actor} le dio like a tu reseña.`);
        break;
      case "comment":
        toast(`${actor} comentó tu reseña.`);
        break;
      case "review":
        toast(`${actor} mencionó una de tus películas.`);
        break;
      case "system":
        toast(notif.message ?? "Nueva notificación.");
        break;
      default:
        toast(notif.message ?? "Nueva notificación.");
    }
