...
import type {
  ...
  UserListSummaryItem,
  WatchlistItem,
} from "../components/profile-v2/models";
import type {
...
export function useProfilePageData(userParam?: string) {
  ...
  const [movieMap, setMovieMap] = useState<Map<number, EnrichedMovie>>(
    new Map(),
  );
  const [userLists, setUserLists] = useState<UserListSummaryItem[]>([]);
  const [signature, setSignature] = useState<CinematicSignatureData | null>(
    null,
  );
  const [curatedGalleryItems, setCuratedGalleryItems] = useState<
    CuratedGalleryItemData[]
  >([]);
    ...
    const load = async () => {
      ...
      try {
        ...
        const [
          ...
          fetchVaultSocial(resolvedTargetId, authToken, isSelf).catch(() => ({
            items: [],
          })),
        ]);
  ...
  const stats: ProfileStatsData = useMemo(
    () => ({
      ...
      following:
        profile?._count?.follows_follows_follower_idTousers ?? following.length,
      followers:
        profile?._count?.follows_follows_following_idTousers ??
        followers.length,
    }),
    [
      profile,
  ...
  const followerUsers: ProfileConnection[] = useMemo(
    () =>
      followers.map((item) => ({
        id: item.id,
  ...
  const followingUsers: ProfileConnection[] = useMemo(
    () =>
      following.map((item) => ({
        id: item.id,
        username: item.username,
  ...
  const reviewItems: ReviewItem[] = useMemo(
    () =>
      writtenReviews.slice(0, 12).map((entry) => {
        const fromMovieMap = movieMap.get(entry.movie_id);
        const fromDiary = diary.find(
          (diaryEntry) => diaryEntry.movie_id === entry.movie_id,
        );
        const fromWatchlist = watchlist.find(
