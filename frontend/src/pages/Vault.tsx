...
import {
  Heart, MessageCircle, Bookmark, Play, Plus,
  ArrowLeft, Crown,
} from 'lucide-react';
...
const I = {
  ...
  portrait:      'https://images.unsplash.com/photo-1761429944940-fe98ec7ba4cb?w=800&q=80',
  ...
  audioWave:     'https://images.unsplash.com/photo-1765408217331-6d73ee0fc260?w=800&q=80',
  filmReel:      'https://images.unsplash.com/photo-1770982726697-309881d78cc1?w=800&q=80',
  darkRain:      'https://images.unsplash.com/photo-1741079746677-5f25b2de7fa0?w=800&q=80',
  desolate:      'https://images.unsplash.com/photo-1691573252567-6c1be35aae79?w=800&q=80',
  avatar:        'https://images.unsplash.com/photo-1628070435838-19eb835ad70d?w=200&q=80',
};
...
const ENTRIES: VaultEntry[] = [
  ...
  },
  {
    ...
    film: 'Stalker',
    likes: 98, comments: 14,
    duration: '14 min',
    img: I.audioWave,
  },
  {
    id: 11, type: 'moodboard',
    title: 'Neorrealismo italiano — palette visual',
    film: undefined,
    likes: 201, comments: 33,
  ...
  },
  {
    ...
    film: undefined,
    likes: 389, comments: 71,
    posterCount: 8,
    posters: [I.portrait, I.fog, I.mistyRoad, I.grain],
  },
];

type FilterType = 'TODO' | 'VIDEOS' | 'IMÁGENES' | 'AUDIOS' | 'MOOD BOARDS' | 'LISTAS' | 'RESEÑAS';
...
function fromReviewsToVault(reviews: ReviewEntry[]): VaultEntry[] {
  return reviews.slice(0, 8).map((review) => ({
    ...
    film: review.movies_ref?.tmdb_id ? `TMDB ${review.movies_ref.tmdb_id}` : undefined,
...
function fromDiaryToVault(diary: RichDiaryEntry[]): VaultEntry[] {
  ...
  visuals.forEach((item, idx) => {
    ...
    const collage = [
      image,
      toTmdbImage(diary[(idx + 1) % visuals.length]?.movie_info?.poster_path || null),
      toTmdbImage(diary[(idx + 2) % visuals.length]?.movie_info?.poster_path || null),
      I.blueTexture,
    ];
  ...
  if (visuals.length > 0) {
    entries.push({
      ...
      posters: visuals.slice(0, 5).map((item) => toTmdbImage(item.movie_info?.poster_path || null)),
    });
  }
...
function CardFooter({ entry, avatar }: { entry: VaultEntry; avatar: string }) {
  const typeLabel: Record<EntryType, string> = {
    ...
    moodboard: 'Mood Board', list: 'Lista', review: 'Reseña',
  };
  ...
  return (
    <div style={{ padding: '10px 14px', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {entry.film && (
        <span style={{ fontSize: 10, color: C.accent, fontFamily: SANS, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{entry.film}</span>
      )}
      <span style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: C.textMuted, border: `1px solid ${C.border}`, padding: '2px 7px', fontFamily: SANS, flexShrink: 0 }}>{typeLabel[entry.type]}</span>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 'auto', flexShrink: 0 }}>
        <button onClick={() => setLiked(v => !v)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, color: liked ? C.accent : C.textSoft, padding: 0, transition: 'color 0.2s' }}>
          <Heart size={11} fill={liked ? C.accent : 'none'} />
          <span style={{ fontSize: 10, fontFamily: SANS }}>{entry.likes + (liked ? 1 : 0)}</span>
        </button>
        <button style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, color: C.textSoft, padding: 0 }}>
          <MessageCircle size={11} />
          <span style={{ fontSize: 10, fontFamily: SANS }}>{entry.comments}</span>
        </button>
      </div>
    </div>
  );
...
function ImageCard({ entry, avatar }: { entry: VaultEntry; avatar: string }) {
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: 12 }}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}>
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        <Img src={entry.img} alt={entry.title} style={{ width: '100%', display: 'block', filter: hov ? 'brightness(0.65)' : 'brightness(0.85) saturate(0.7)', transition: 'filter 0.35s', transform: hov ? 'scale(1.03)' : 'scale(1)', transformOrigin: 'center', }} />
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(8,8,8,0.72)', opacity: hov ? 1 : 0, transition: 'opacity 0.3s', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 16 }}>
          <div style={{ fontFamily: SERIF, fontSize: 16, fontStyle: 'italic', color: C.text, lineHeight: 1.35 }}>{entry.title}</div>
          {entry.film && <div style={{ fontSize: 11, color: C.accent, fontFamily: SANS, marginTop: 4 }}>{entry.film}</div>}
        </div>
      </div>
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: 12 }}
      ...
      <div style={{ position: 'relative', aspectRatio: '16/9', overflow: 'hidden', cursor: 'pointer' }}>
        <Img src={entry.img} alt={entry.title} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(0.4) brightness(0.55)', transform: hov ? 'scale(1.03)' : 'scale(1)', transition: 'all 0.35s' }} />
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: 44, height: 44, borderRadius: '50%', border: `1.5px solid ${hov ? C.accent : 'rgba(255,255,255,0.4)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)', background: hov ? C.accentGlow : 'rgba(255,255,255,0.06)', transition: 'all 0.25s' }}>
            <Play size={14} fill="white" color="white" style={{ marginLeft: 2 }} />
          </div>
        </div>
        {entry.duration && (
          <div style={{ position: 'absolute', bottom: 8, right: 10, fontSize: 10, color: C.text, fontFamily: SANS, background: 'rgba(8,8,8,0.85)', padding: '2px 6px' }}>{entry.duration}</div>
        )}
      </div>
      <div style={{ padding: '12px 14px 4px' }}>
        <div style={{ fontFamily: SERIF, fontSize: 15, color: C.text, lineHeight: 1.35, marginBottom: 2 }}>{entry.title}</div>
      </div>
      <CardFooter entry={entry} avatar={avatar} />
    </div>
  );
...
function AudioCard({ entry, avatar }: { entry: VaultEntry; avatar: string }) {
  ...
  return (
    <div style={{ background: C.elevated, border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: 12, position: 'relative' }}>
      <Img src={entry.img} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.15) saturate(0)', pointerEvents: 'none' }} />
      <div style={{ position: 'relative', zIndex: 1, padding: '20px 18px 4px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <button onClick={toggle} style={{ width: 36, height: 36, borderRadius: '50%', border: `1.5px solid ${C.accent}`, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', cursor: 'pointer', flexShrink: 0, color: C.accent }}>
            {playing
              ? <svg width="12" height="12" viewBox="0 0 12 12" fill={C.accent}><rect x="2" y="2" width="3" height="8" rx="0.5"/><rect x="7" y="2" width="3" height="8" rx="0.5"/></svg>
          ...
          <div style={{ flex: 1, display: 'flex', gap: 2, alignItems: 'center', height: 28 }}>
            {Array.from({ length: 28 }).map((_, i) => {
              ...
              return <div key={i} style={{ flex: 1, height: `${h}px`, background: filled ? C.accent : C.textMuted, borderRadius: 1, transition: 'background 0.1s', opacity: filled ? 1 : 0.5 }} />;
            })}
          </div>
          {entry.duration && <span style={{ fontSize: 10, color: C.textSoft, fontFamily: SANS, flexShrink: 0 }}>{entry.duration}</span>}
        </div>
        <div style={{ fontFamily: SERIF, fontSize: 17, color: C.text, lineHeight: 1.35, marginBottom: 4 }}>{entry.title}</div>
      </div>
      <CardFooter entry={entry} avatar={avatar} />
    </div>
  );
...
function MoodboardCard({ entry, avatar }: { entry: VaultEntry; avatar: string }) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
        {imgs.slice(0, 6).map((src, i) => (
          <div key={i} style={{ aspectRatio: '1', overflow: 'hidden' }}>
            <Img src={src} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(0.5) brightness(0.75)', transition: 'filter 0.3s' }}
              onMouseEnter={e => { (e.target as HTMLImageElement).style.filter = 'saturate(0.8) brightness(0.9)'; }}
              onMouseLeave={e => { (e.target as HTMLImageElement).style.filter = 'saturate(0.5) brightness(0.75)'; }}
            />
          </div>
        ))}
      </div>
      <div style={{ padding: '12px 14px 4px' }}>
        <div style={{ fontFamily: SERIF, fontSize: 15, color: C.text, lineHeight: 1.35 }}>{entry.title}</div>
      </div>
      <CardFooter entry={entry} avatar={avatar} />
    </div>
  );
    ...
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: 12 }}>
      ...
      <div style={{ padding: '20px 16px 8px', display: 'flex', justifyContent: 'center' }}>
        <div style={{ position: 'relative', width: 80, height: 110 }}>
          {posters.slice(0, 3).reverse().map((src, i) => {
            const idx = 2 - i;
            return (
              <div key={i} style={{
                position: 'absolute', width: 72, height: 100,
                top: idx * 4, left: idx * 4,
                zIndex: idx + 1,
      ...
      <div style={{ padding: '8px 14px 4px' }}>
        <div style={{ fontFamily: SERIF, fontSize: 17, color: C.text, lineHeight: 1.35, marginBottom: 4 }}>{entry.title}</div>
        <div style={{ fontSize: 10, color: C.textSoft, fontFamily: SANS }}>{entry.posterCount} películas</div>
      </div>
      <CardFooter entry={entry} avatar={avatar} />
    </div>
  );
    ...
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: 12 }}>
      {entry.img && (
        <div style={{ height: 80, overflow: 'hidden', position: 'relative' }}>
          <Img src={entry.img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(0.3) brightness(0.4)' }} />
          {entry.film && (
            <div style={{ position: 'absolute', bottom: 10, left: 14, fontFamily: SANS, fontSize: 11, color: C.accent, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{entry.film}</div>
          )}
        </div>
      )}
      <div style={{ padding: '14px 16px 4px' }}>
        <div style={{ fontFamily: SERIF, fontSize: 17, color: C.text, lineHeight: 1.3, marginBottom: 10 }}>{entry.title}</div>
        <div style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 14, color: C.textSoft, lineHeight: 1.7, display: '-webkit-box', WebkitLineClamp: 4, WebkitBoxOrient: 'vertical' as const, overflow: 'hidden' }}>
          {entry.text}
        </div>
      </div>
      <CardFooter entry={entry} avatar={avatar} />
...
function EntryCard({ entry, avatar }: { entry: VaultEntry; avatar: string }) {
  switch (entry.type) {
    case 'image':    return <ImageCard    entry={entry} avatar={avatar} />;
    case 'video':    return <VideoCard    entry={entry} avatar={avatar} />;
    case 'audio':    return <AudioCard    entry={entry} avatar={avatar} />;
    case 'moodboard':return <MoodboardCard entry={entry} avatar={avatar} />;
    case 'list':     return <ListCard     entry={entry} avatar={avatar} />;
    case 'review':   return <ReviewCard   entry={entry} avatar={avatar} />;
  }
}
...
export function Vault() {
  ...
  const filters: FilterType[] = ['TODO', 'VIDEOS', 'IMÁGENES', 'AUDIOS', 'MOOD BOARDS', 'LISTAS', 'RESEÑAS'];
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const profile = username ? await fetchUserProfileByUsername(username, token) : null;
        const viewer = await getCurrentUser().catch(() => null);

        if (!active) return;

        const owner = Boolean(viewer && profile && viewer.username.toLowerCase() === profile.username.toLowerCase());
        setIsOwner(owner);
    ...
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: SANS }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}
        ...
        <div style={{ width: 88, height: 88, flexShrink: 0, position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', bottom: -6, right: -6, background: C.accent, color: C.bg, fontFamily: SANS, fontSize: 8, letterSpacing: '0.15em', padding: '3px 7px', display: 'flex', alignItems: 'center', gap: 3 }}>
        ...
        {/* Info */}
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: SERIF, fontSize: 38, fontWeight: 400, color: C.text, lineHeight: 1, marginBottom: 6 }}>{user.name}</div>
          <div style={{ fontSize: 12, color: C.textSoft, fontFamily: SANS, marginBottom: 8, letterSpacing: '0.1em' }}>@{user.username}</div>
          <div style={{ fontSize: 13, color: C.textSoft, fontFamily: SERIF, fontStyle: 'italic' }}>{user.bio}</div>
        </div>

        {/* Stats + actions */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 14 }}>
          <div style={{ textAlign: 'right' }}>
            <span style={{ fontFamily: SERIF, fontSize: 30, fontWeight: 300, color: C.text }}>{user.entries}</span>
            <span style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: C.textSoft, marginLeft: 8, fontFamily: SANS }}>entradas</span>
          </div>
          {!isOwner ? (
            <button style={{ padding: '9px 22px', background: C.accent, color: C.bg, border: 'none', fontFamily: SANS, fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', cursor: 'pointer' }}>
              Seguir
            </button>
          ) : (
            <div style={{ fontSize: 11, color: C.textMuted, fontFamily: SANS, fontStyle: 'italic' }}>Tu vault</div>
          )}
        </div>
      </motion.div>
        ...
        <div className="vault-type-tabs" style={{ gap: 0 }}>
          {filters.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)} style={{
              ...
              fontFamily: SANS, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase',
      ...
      <div className="vault-content">
        ...
        {loadError && (
          <div style={{ color: '#C97B7B', fontFamily: SANS, fontSize: 12, marginBottom: 16 }}>{loadError}</div>
        )}
              ...
              <ResponsiveMasonry columnsCountBreakPoints={{ 640: 2, 1024: 3 }}>
                <Masonry gutter="0px">
                  {filtered.map((entry, i) => (
                    <motion.div key={entry.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} style={{ padding: '0 6px' }}>
      ...
      {isOwner && (
        <motion.button
          ...
          style={{
            ...
            padding: '13px 22px', background: C.accent, color: C.bg,
            border: 'none', fontFamily: SANS, fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase',
