    ...
    >
      ...
      <div>
        <div
          style={{
            fontFamily: SERIF,
            fontSize: 22,
            color: film.watched ? C.textSoft : C.text,
            lineHeight: 1.2,
            marginBottom: 4,
          }}
        >
      ...
      <div style={{ paddingTop: 4 }}>
        {canMark ? (
          ...
          <span style={{ color: C.textMuted, fontSize: 10, fontFamily: SANS }}>
            Inicia sesion para marcar
          </span>
        )}
      </div>
    </motion.div>
...
export function ArcoDetail() {
  ...
  const watchedCount = useMemo(
    () => films.filter((f) => f.watched).length,
    [films],
  );
  const totalFilms = films.length;
  const progressPct =
    totalFilms > 0 ? Math.round((watchedCount / totalFilms) * 100) : 0;
  ...
  const onMark = async (movieId: number) => {
    ...
    try {
      ...
      setError(
        "No se pudo marcar progreso. Verifica sesion y vuelve a intentar.",
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "56px 48px 0", maxWidth: 1080, margin: "0 auto" }}>
        ...
        {error && (
          <div style={{ color: "#C97B7B", marginBottom: 10 }}>{error}</div>
        )}
          ...
          >
            ...
            >
              ...
              >
                ...
                {Math.max(2, Math.round((detail.films || 1) * 2.2))}h estimadas
              </span>
              <span
                style={{
                  fontSize: 11,
              ...
              >
                <Users size={10} color={C.textMuted} />{" "}
                {detail.usersCompleted.toLocaleString("es")} lo completaron
              </span>
            </div>
            ...
            {!joined ? (
              ...
              <div style={{ maxWidth: 400 }}>
                ...
                >
                  <span
                    style={{ fontSize: 12, color: C.text, fontFamily: SANS }}
                  >
                    Llevas <strong>{watchedCount}</strong> de{" "}
                    <strong>{totalFilms}</strong> peliculas.
                  </span>
                  <span
                    style={{ fontSize: 12, color: C.accent, fontFamily: SANS }}
                  >
                    {progressPct}%
                  </span>
                </div>
                <div
                  style={{ height: 3, background: C.border, borderRadius: 2 }}
                >
                  <motion.div
                    initial={{ width: 0 }}
      ...
      {joined && watchedCount > 0 && detail && (
        <div
          style={{ maxWidth: 1080, margin: "40px auto 0", padding: "0 48px" }}
        >
          <motion.div
          ...
          >
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              ...
              <div style={{ fontFamily: SERIF, fontSize: 18, color: C.text }}>
                Ya llevas {watchedCount}{" "}
                {watchedCount === 1 ? "pelicula" : "peliculas"} de este arco. Te
                quedan {Math.max(0, totalFilms - watchedCount)}.
              </div>
            </div>
          </motion.div>
        </div>
      ...
      >
        <div style={{ borderTop: `1px solid ${C.border}` }}>
          {films.length === 0 && !loading ? (
            <div style={{ color: C.textSoft, padding: "18px 0" }}>
              Este arco todavia no tiene peliculas cargadas.
            </div>
          ) : (
            films.map((film, i) => (
              <FilmEntry
