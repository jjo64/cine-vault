...
function PosterFan({ films, type }: { films: FilmEntry[]; type: SessionType }) {
  ...
  const transforms: Record<
    number,
    { rotate: number; tx: number; ty: number; z: number }[]
  > = {
  ...
  return (
    <div
      className="diary-poster-fan-container"
      style={{ width: containerW + 40, height: posterH + 24 }}
    >
      {films.slice(0, 4).map((film, i) => {
        ...
        return (
          <div
            key={film.id}
            className="diary-poster-fan-item"
            style={{
              top: cfg.ty,
              left: "50%",
              width: posterW,
...
function SessionCard({
  ...
  return (
    ...
    >
      ...
      <div
        className="diary-session-layout"
        style={{ paddingTop: label ? 28 : 0 }}
      >
        {/* Abanico */}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {/* Date */}
          <div
            className="diary-session-date-col"
            style={{
              fontSize: 11,
              color: C.textSoft,
              fontFamily: SANS,
        ...
        .then((res) => {
          setSessions(
            (res.sessions || []).map((s: any) => ({
              ...s,
              date: new Date(s.dateISO).toLocaleDateString("es-AR", {
                year: "numeric",
                month: "long",
                day: "numeric",
              }),
  ...
  return (
    ...
    >
      ...
      <nav
        ...
        style={{ borderBottom: `1px solid ${C.border}`, gap: 20 }}
      >
        <Link
          to="/profile"
          style={{
      ...
      <div className="diary-main" style={{ paddingTop: 56 }}>
        ...
        >
          ...
          >
            No es un historial. Es el registro de cuándo viste qué y con qué
            estado de ánimo. Las Dobles Funciones y Maratones tienen su propio
            formato.
          </div>
        </motion.div>
      </div>
      ...
      >
        ...
        {!loading &&
          filtered.map((session, i) => {
            const isNewMonth =
              i > 0 &&
              new Date(session.dateISO).getMonth() !==
                new Date(filtered[i - 1].dateISO).getMonth();
            return (
              <div key={session.id}>
                {isNewMonth && (
                  <div
                  ...
                  >
                    ...
                    >
                      {new Date(session.dateISO).toLocaleString("es-AR", {
                        month: "long",
                        year: "numeric",
                      })}
                    </span>
