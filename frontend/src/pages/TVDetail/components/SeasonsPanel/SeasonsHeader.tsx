...
export default function SeasonHeader({ season, watched, pct }: SeasonHeaderProps) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 28, marginBottom: 32 }}>
      <div style={{ aspectRatio: '2/3', borderRadius: 1, overflow: 'hidden', border: `1px solid ${C.border}` }}>
        <Img
          ...
          alt={season.name}
          style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(0.5) brightness(0.75)' }}
        />
      </div>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: SERIF, fontSize: 22, color: C.text }}>{season.name}</span>
          {season.air_date && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ flex: 1, maxWidth: 200, height: 2, background: C.border, borderRadius: 1 }}>
            <div style={{ height: '100%', width: `${pct}%`, background: C.accent, borderRadius: 1, transition: 'width 0.4s' }} />
          </div>
          <span style={{ fontSize: 10, color: C.textSoft, fontFamily: SANS }}>
