...
type ViewerMeta = { id: number; membership?: string | null; role?: string | null }
...
export function useUserActions(detailId: number | undefined) {
  ...
  const mapReviewToForm = (review?: AppReview | null) => ({
    ...
    dimensions: {
      direccion: review?.rating_direccion != null ? Number(review.rating_direccion) : null,
      guion: review?.rating_guion != null ? Number(review.rating_guion) : null,
      fotografia: review?.rating_fotografia != null ? Number(review.rating_fotografia) : null,
      actuaciones: review?.rating_actuaciones != null ? Number(review.rating_actuaciones) : null,
      bandaSonora: review?.rating_banda_sonora != null ? Number(review.rating_banda_sonora) : null,
    },
    liked: isFavorite,
    seenDate: new Date().toISOString().slice(0, 10),
    seenBefore: false,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setViewer({ id: currentUser.id, membership: currentUser.membership || null, role: currentUser.role || null })
        ...
        const withUsers = await Promise.all(rawRev.slice(0, 20).map(async r => {
          try {
            const u = await fetchUserById(r.user_id)
            return { ...r, username: u.username || `user-${r.user_id}` }
          } catch {
  ...
  const handleWriteReview = () => {
    if (!detailId) return
    const ownReview = reviews.find((review) => viewer && review.user_id === viewer.id)
