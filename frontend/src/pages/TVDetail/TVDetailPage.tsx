...
function Hero({
  ...
  const yearEnd =
    detail.in_production === false
      ? formatYear(detail.last_air_date)
      : "presente";
  const years =
    yearStart === yearEnd || !detail.last_air_date
      ? yearStart
      : `${yearStart} – ${yearEnd}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Img
            src={img(detail.poster_path, TMDB_POSTER)}
            alt={detail.name}
            style={{
              width: "100%",
              height: "100%",
        ...
        >
          ...
          {detail.vote_count != null && (
            <div style={{ fontSize: 11, color: C.textMuted, fontFamily: SANS }}>
              {detail.vote_count.toLocaleString("es-ES")} ratings
            </div>
          )}
        </div>
      ...
      >
        {cast.map((member) => (
          ...
          >
            {member.profile_path ? (
              <img
                src={img(member.profile_path, TMDB_THUMB)}
                alt={member.name}
                style={{
                  width: 36,
                  height: 36,
...
function CrewSection({ detail }: { detail: TVDetailApi }) {
  ...
  const crew = useMemo(() => {
    ...
    return Array.from(map.entries())
      .slice(0, 5)
      .map(([dept, members]) => ({ dept, members: members.slice(0, 8) }));
  }, [detail.credits?.crew]);
                ...
                <div style={{ display: "flex", gap: 2 }}>
                  {Array.from({ length: 5 }).map((_, j) => (
                    <svg
                      ...
                      viewBox="0 0 12 12"
                      fill={
                        j < Number(review.rating || 0) ? C.gold : C.textMuted
                      }
                    >
      ...
      <div style={{ marginTop: 32 }}>
        {!writerOpen ? (
          ...
          >
            ...
            {myReviewId ? "Editar reseña" : "Escribir reseña"}
          </button>
        ) : (
          <motion.div
...
function TechnicalSheet({ detail }: { detail: TVDetailApi }) {
  ...
  const rows = [
    {
      label: "Creada por",
      value: (detail.created_by || []).map((c) => c.name).join(", "),
    },
    {
      label: "Red",
      value: (detail.networks || []).map((n) => n.name).join(" / "),
    },
    {
      label: "País",
      value: (detail.production_countries || []).map((c) => c.name).join(", "),
    },
    {
      label: "Idioma",
    ...
    {
      label: "Temporadas",
      value: detail.number_of_seasons ? String(detail.number_of_seasons) : "",
    },
    {
      label: "Episodios",
      value: detail.number_of_episodes ? String(detail.number_of_episodes) : "",
    },
