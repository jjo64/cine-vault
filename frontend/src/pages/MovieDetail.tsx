...
import {
  ...
  getDirectorObj,
} from "../features/movie-detail/utils/mapping";
...
import {
  ...
  TMDB_BASE,
  SIZES,
} from "../features/movie-detail/constants";
...
export default function MovieDetailPage() {
  ...
  const seo = useMemo(() => {
    ...
    const year = movie.release_date
      ...
      : null;
    const title = year
      ? `${movie.title} (${year}) | CineVault`
      : `${movie.title} | CineVault`;
    const description = (
      movie.overview ||
      movie.tagline ||
      `Descubre ${movie.title} en CineVault.`
    ).slice(0, 155);
    const image = movie.backdrop_path
      ? `${TMDB_BASE}w1280${movie.backdrop_path}`
      : `${TMDB_BASE}w780${movie.poster_path}`;
  ...
  return (
    ...
    >
      ...
      <Hero
        ...
        runtime={
          movie.runtime ? `${movie.runtime} min` : "Duración desconocida"
        }
        genresText={
          (movie.genres || [])
            .slice(0, 2)
      ...
      <div className="md-main-layout">
        ...
        <Sidebar
          ...
          similar={similar}
          directorObj={
            directorObj ? { id: directorObj.id, name: directorObj.name } : null
          }
        />
      </div>
