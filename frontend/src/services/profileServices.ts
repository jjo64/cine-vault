...
export async function resolveViewerId(): Promise<{
  userId: number | null;
...
export const fetchUserProfileByUsername = (
  username: string,
  token?: string | null,
) =>
  apiFetch<ProfileUser | null>(
...
export const fetchWatchlist = (
  ...
  apiFetch<RichWatchlistEntry[]>(
    isSelf ? "/api/watchlist" : `/api/watchlist/${userId}`,
    {
      token: isSelf ? token : undefined,
      defaultValue: [],
    },
...
export const fetchFavorites = (
  ...
  apiFetch<FavoriteEntry[]>(
    isSelf ? "/api/favorites" : `/api/favorites/user/${userId}`,
    {
      token: isSelf ? token : undefined,
      defaultValue: [],
    },
...
export const fetchReviews = (
  ...
  apiFetch<ReviewEntry[]>(
    isSelf ? "/api/reviews" : `/api/reviews/user/${userId}`,
    {
      token: isSelf ? token : undefined,
      defaultValue: [],
    },
...
export const checkUsernameAvailability = async (username: string) => {
  ...
  const response = await fetch(
    `${API_URL}/api/users/username/${encodeURIComponent(candidate)}`,
  );
...
export const updateAvatarSettings = (
  token: string | null | undefined,
  avatar: string,
) =>
...
export const fetchOwnerCinematicSignature = (
  token: string | null | undefined,
) =>
  apiFetch<SettingsWrappedResponse<CinematicSignatureData>>(
    "/api/settings/profile/signature",
    {
      token,
      defaultValue: {
        ok: true,
...
export const fetchOwnerCuratedGallery = (token: string | null | undefined) =>
  apiFetch<SettingsWrappedResponse<{ items: CuratedGalleryItemData[] }>>(
    "/api/settings/profile/curated-gallery",
    {
      token,
...
export const fetchPublicCuratedGallery = (userId: number) =>
  apiFetch<{ items: CuratedGalleryItemData[] }>(
    `/api/users/${userId}/profile/curated-gallery`,
    {
      defaultValue: { items: [] },
    },
  );
