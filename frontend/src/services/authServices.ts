...
const API_URL = String(
  ...
  .trim()
  .replace(/\/+$/, "");
const GOOGLE_REDIRECT_URI_ENV = String(
  import.meta.env.VITE_GOOGLE_REDIRECT_URI || "",
).trim();
const GOOGLE_REDIRECT_URI_PROD =
  "https://cine-vault-ncuh.onrender.com/api/auth/google/callback";
const GOOGLE_REDIRECT_URI_DEV =
  "http://localhost:4000/api/auth/google/callback";
const ACCESS_TOKEN_KEY = "token";
const AUTH_STORAGE_MODE_RAW = String(
  import.meta.env.VITE_AUTH_STORAGE_MODE || "hybrid",
).toLowerCase();
...
export const notifyAuthStateChanged = (authenticated: boolean) => {
  ...
  window.dispatchEvent(
    new CustomEvent("auth-state-changed", { detail: { authenticated } }),
  );
};
...
export async function activateTwoFactor(): Promise<TwoFactorActivateResponse> {
  ...
  if (!response.ok) {
    throw new Error(
      await parseApiErrorMessage(response, "No se pudo generar el QR de 2FA"),
    );
  }
...
export async function confirmTwoFactor(
  ...
  if (!response.ok) {
    throw new Error(
      await parseApiErrorMessage(response, "No se pudo confirmar 2FA"),
    );
  }
...
export async function disableTwoFactor(codigo: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      await parseApiErrorMessage(response, "No se pudo desactivar 2FA"),
    );
  }
}
...
export async function getRecoveryCodesStatus(): Promise<RecoveryCodesStatusResponse> {
  const response = await authorizedFetch(
    "/api/auth/2fa/recovery-codes/status",
    {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    },
  ...
  if (!response.ok) {
    throw new Error(
      await parseApiErrorMessage(
        response,
        "No se pudo obtener estado de recovery codes",
      ),
    );
  }
...
export async function regenerateRecoveryCodes(
  ...
  const response = await authorizedFetch(
    "/api/auth/2fa/recovery-codes/regenerar",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ codigo }),
  ...
  if (!response.ok) {
    throw new Error(
      await parseApiErrorMessage(
        response,
        "No se pudieron regenerar los recovery codes",
      ),
    );
  }
