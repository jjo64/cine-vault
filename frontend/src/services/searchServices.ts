...
async function runSearch(
  ...
  const response = await fetch(
    `${API_URL}${scopePath[scope]}?${search.toString()}`,
  );
  if (!response.ok) {
...
export async function searchMoviesDebug(
  query: string,
  page = 1,
): Promise<SearchResponse> {
...
export const searchPerson = (query: string, page = 1) =>
  runSearch("person", { query, page });

export const searchTV = (query: string, page = 1) =>
  runSearch("tv", { query, page });
...
export async function searchUsers(
  ...
  const response = await fetch(
    `${API_URL}/api/users/search?q=${encodeURIComponent(q)}&limit=${limit}`,
  );
  if (!response.ok) return [];
