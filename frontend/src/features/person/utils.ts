...
export function getDecades(items: CreditItem[]) {
  ...
  return [
    "Todo",
    ...unique.sort((a, b) => Number.parseInt(b, 10) - Number.parseInt(a, 10)),
  ];
}
...
export function splitCrew(items: CreditItem[]) {
  return {
    director: items.filter(
      (item) => (item.job || "").toLowerCase() === "director",
    ),
...
export function computeStats(
  personPopularity: number | undefined,
  knownFor: CreditItem[],
  castCount: number,
  crewCount: number,
): PersonStats {
  ...
  const avgRating =
    totalFilms > 0
      ? knownFor.reduce(
          (acc, item) => acc + Number(item.vote_average || 0),
          0,
        ) / totalFilms
      : 0;
