...
export function PersonPageView() {
  ...
  const stats = useMemo(
    () => computeStats(person?.popularity, knownFor, castMovies.length, crewMovies.length),
  ...
  const heroBackdrop = useMemo(() => toPoster(knownFor[0]?.poster_path, 'original'), [knownFor])
  const portrait = useMemo(() => toPoster(person?.profile_path, 'original'), [person?.profile_path])

  const bioParagraphs = useMemo(() => getBiographyParagraphs(person?.biography), [person?.biography])
  ...
  const departmentLabel = DEPARTMENT_LABELS[person.known_for_department || ''] || person.known_for_department || 'Persona'
    ...
    <div className={styles.root}>
      ...
      <div className={styles.pagePadTop}>
        <section className={styles.hero}>
          ...
          >
            <Img src={portrait} alt={person.name} className={styles.heroPortrait} />
          ...
          >
            ...
            <div className={styles.heroMetaRow}>
              <div className={styles.heroMetaItem}><Calendar size={12} color="#9A7A48" /> {person.birthday || 'Sin fecha'}{person.deathday ? ` — ${person.deathday}` : ''}</div>
              <div className={styles.heroMetaItem}><MapPin size={12} color="#9A7A48" /> {person.place_of_birth || 'Lugar desconocido'}</div>
            </div>

            <div className={styles.statsRow}>
              <div><div className={styles.statValue}>{stats.fans.toLocaleString()}</div><div className={styles.statLabel}>Fans estimados</div></div>
              <div><div className={styles.statValue}>{stats.avgRating ? stats.avgRating.toFixed(1) : '-'}</div><div className={styles.statLabel}>Rating medio</div></div>
              <div><div className={styles.statValue}>{stats.totalFilms}</div><div className={styles.statLabel}>Películas destacadas</div></div>
            </div>
            ...
            <div className={styles.actionsRow}>
              ...
              <button className={styles.iconButton}><Share2 size={15} /></button>
            </div>
          </motion.div>
        </section>
        ...
        <div className={styles.contentWrap}>
          <div className={styles.twoCols}>
            <main>
              <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className={styles.sectionBlock}>
                <div className={styles.sectionLabel}>Biografía <div className={styles.sectionLine} /></div>
                {visibleBio.map((paragraph, index) => (
                  <p key={index} className={styles.biography}>{paragraph}</p>
                ))}
                {bioParagraphs.length > 1 ? (
                  <button onClick={() => setBioExpanded((value) => !value)} className={styles.toggleBioButton}>
              ...
              <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className={styles.sectionBlock}>
                <div className={styles.sectionLabel}>Conocido por <div className={styles.sectionLine} /></div>
                <div className={styles.knownForRow}>
                  {knownFor.slice(0, 6).map((item) => {
                    const title = getCreditTitle(item)
                    return (
                      <Link key={item.id} to={movieHref(item)} className={styles.knownForLink}>
                        <motion.div whileHover={{ y: -5 }} transition={{ duration: 0.25 }}>
                          <div className={styles.knownForPosterWrap}>
                            <Img src={toPoster(item.poster_path)} alt={title} className={styles.knownForPoster} />
                          </div>
                          <div className={styles.knownForTitle}>{title}</div>
                          <div className={styles.knownForYear}>{getCreditYear(item) || '-'}</div>
                        </motion.div>
                      </Link>
                    )
                  })}
              ...
              <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className={styles.sectionBlock}>
                <div className={styles.sectionLabel}>Filmografía <div className={styles.sectionLine} /></div>
                ...
                {visibleItems.length === 0 ? (
                  <div className={styles.emptyState}>Sin resultados para este filtro.</div>
                ) : (
            ...
            <aside>
              <div className={styles.stickyAside}>
                <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className={styles.awardsSection}>
                  <div className={styles.sectionLabel}>Reconocimientos <div className={styles.sectionLine} /></div>
                  ...
                  <div className={styles.statsCardRows}>
                    {[
                      ...
                      <div key={index} className={styles.statsCardRow}>
                        <div className={styles.statsCardLabel}>{entry.icon} {entry.label}</div>
