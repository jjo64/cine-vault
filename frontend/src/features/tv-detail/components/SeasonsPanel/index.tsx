...
export default function SeasonsPanel({
  ...
  const { watched, pct } = useSeasonProgress(
    watchedIds,
    activeSeason,
    episodes,
  ...
  const handleToggle = (episodeNumber: number) => {
    ...
    onToggleEpisode(episodeKey(activeSeason, episodeNumber));
  };
          ...
          <div>
            {visible.map((ep) => (
              <EpisodeRow
                key={ep.id}
                ep={ep}
                watched={watchedIds.has(
                  episodeKey(activeSeason, ep.episode_number ?? 0),
                )}
