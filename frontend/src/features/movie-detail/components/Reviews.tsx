...
export function Reviews({
  ...
  const buildReviewThreadHref = (review: AppReview) => {
    ...
    const preferredId =
      movieTmdbId != null && Number.isFinite(movieTmdbId)
        ? movieTmdbId
        : review.tmdbId != null && Number.isFinite(review.tmdbId)
          ? review.tmdbId
          : null;
      ...
      {reviews.map((review, index) => {
        ...
        return (
          ...
          >
            ...
            >
              <Link
                to={`/${encodeURIComponent(review.username)}`}
              ...
              <div>
                ...
                >
                  ...
                  {review.tiempoLecturaMin ? (
                    <span>{review.tiempoLecturaMin} min lectura</span>
                  ) : null}
                </div>
              </div>
              <div style={{ marginLeft: "auto", display: "flex", gap: 2 }}>
                {[1, 2, 3, 4, 5].map((value) => (
                  <span
                    ...
                    style={{
                      fontSize: 13,
                      color: value <= review.rating ? C.gold : C.textMuted,
                    }}
                  >
                    ★
                  </span>
            ...
            {(review.timestamps.length > 0 || review.contieneSpoilers) && (
              ...
              >
                ...
                {review.timestamps.map((stamp, stampIndex) => (
                  <span
                    key={`${review.id}-stamp-${stampIndex}`}
                    style={{
                      border: `1px solid ${C.border}`,
                      color: C.textSoft,
                      fontFamily: SANS,
              ...
              >
                ...
                >
                  {REVIEW_DIMENSIONS.map((entry) => {
                    const value =
                      review.dimensions[
                        entry.key as keyof typeof review.dimensions
                      ];
                    return (
                      <div
                        key={`${review.id}-${entry.key}`}
                        style={{
                          fontFamily: SANS,
                          fontSize: 11,
                          color: C.textSoft,
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              ...
              >
                <Heart
                  ...
                  color={liked ? C.accent : "currentColor"}
                />
                {likesCount}
              </button>
              <button
