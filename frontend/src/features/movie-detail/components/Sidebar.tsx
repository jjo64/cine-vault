...
export function Sidebar({ movie, similar, directorObj }: SidebarProps) {
  const photography = getCrewByJob(movie, [
    "Director of Photography",
    "Cinematography",
  ]);
  const music = getCrewByJob(movie, [
  ...
  const metaRows = [
    {
      key: "País",
      val: movie.production_countries?.[0]?.name || "Desconocido",
    },
    {
      key: "Año",
      val: movie.release_date
        ? String(new Date(movie.release_date).getFullYear())
        : "----",
    },
    {
      key: "Duración",
      val: movie.runtime ? `${movie.runtime} min` : "No disponible",
    },
    {
      key: "Idioma",
      val:
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {platforms.map((platform) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="md-sidebar-platforms"
                style={{
                  display: "flex",
                  justifyContent: "space-between",
                  gap: 10,
